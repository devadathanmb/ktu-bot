...
Then something unexpected happened. I started noticing new [chat IDs](https://core.telegram.org/bots/api#chat) popping up in the logs. People were somehow finding this bot despite me not linking it anywhere and actually using it. From there, it spread like wildfire. When results got published, the bot link was shared in almost every group, and it became the first thing people used to check their results - yes, even before the official website.
