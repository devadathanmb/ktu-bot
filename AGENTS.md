...
A GrammY-based Telegram bot serving KTU (Kerala Technological University) students. The system consists of a long-polling bot and several BullMQ background workers, all orchestrated via Docker Compose. PostgreSQL (Drizzle ORM) is used for persistence, and Redis powers the BullMQ job queues.
...
- **ESLint**: Flat config with `typescript-eslint` type-checked rules. `no-floating-promises: error`, `no-unused-vars` (with `_` prefix exception), `no-explicit-any: warn`.
- **Do not introduce `any`**. Type everything properly - use `unknown` if the type is truly unknown and narrow it with type guards.
- **Caching**: The cached Got client is in `src/api/client.ts`. KTU service functions that need cache control accept an optional `apiClient?: Got` parameter (dependency injection). Workers that need fresh data pass `baseApiClient` to bypass the in-memory cache. Never pass `cache: false` to Got's built-in cache option — use the `apiClient` parameter instead.
...
- **Barrel exports**: Use `index.ts` for directories that intentionally expose a public API. Not every implementation-only directory has or needs a barrel.

### Config Validation

Environment variables should be validated at startup using **Zod v4** schemas in `src/configs/`. The pattern:
...
Use `.superRefine()` for cross-field validation and `.transform()` for derived values. Do not add new direct `process.env` reads outside config modules; if touching older exceptions, prefer moving them behind a config schema.

### API Layer and Caching

The project uses [Got](https://github.com/sindresorhus/got) as its HTTP client for KTU API calls. The API layer is organized under `src/api/`:
...
- **`hooks/`** — Adds the request headers KTU expects and centralizes API request/response behavior.
- **`services/`** — KTU service functions use `cachedApiClient` by default. Services that need cache injection accept `apiClient?: Got`; workers pass `baseApiClient` when they need fresh data. Attachment download endpoints use the cached client directly but are excluded by cache configuration.
- **`cache/`** — In-memory caching layer using `lru-cache`. Bypasses Got's HTTP-semantics-based caching (which doesn't work with KTU's non-standard APIs) in favor of custom URL+body-hash cache keys with per-endpoint TTLs. Only 200 responses are cached; non-200 responses invalidate the cache entry. Large attachment endpoints are excluded.
...
Use **pino** (`import logger from "../../utils/logger.js"`). It has serializers for `err` and `error` keys plus redaction for sensitive request fields.
...
- **Main bot plugins**: The main bot currently wires `commands`, `emoji`, `hydrate`, `parse-mode` usage, sessions, and `runner`/`sequentialize` for long polling. Rate limiting middleware is present as a dependency but currently disabled in `src/bot/bot.ts`.
- **Worker bots**: Most workers use `createWorkerBot()` and raw `bot.api`. The announcements notify worker creates a bot instance with `apiThrottler()` and `autoRetry()` because it uploads announcement attachments before enqueueing broadcasts.
- **API calls**: In bot/composer code, use `ctx.api`. In workers, use the worker bot's raw `bot.api`/`this.getBot().api` unless middleware is intentionally configured.
- **Media groups**: If adding `sendMediaGroup`, remember Telegram allows max 10 files per call and captions only work on individual media entries. Current attachment delivery sends documents individually and uses links for files above Telegram's upload limit.
- **Rate limiting**: Telegram rate limits are undocumented. The broadcasts worker uses `concurrency: 1` to handle them safely. When you hit a `retry_after` error in a worker, pause the queue and re-throw for BullMQ retry.
